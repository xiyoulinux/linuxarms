 ...
 */
boolean asthread_init(struct asthread_struct *asthread_nouse,
		      struct assinfo_struct *ssinfo,
		      struct asprocess_struct *sprocess,
		      struct asthread_trans *trans,
		      struct anet_struct *socket)
{
	if (!asthread || !ssinfo || !sprocess || 
	    !trans || !socket || !timer) {
		debug_where();
	...
	asthread->ssinfo   = ssinfo;
	asthread->sprocess = sprocess;
